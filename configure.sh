...
gen_file_lists() {
	...
	needle_src="${typ}_SRC"
	needle_obj="${typ}_OBJ"
	needle_gcda="${typ}_GCDA"
	needle_gcno="${typ}_GCNO"
...
contents=$(gen_file_lists "$contents" "$scriptdir/src/bc" "BC" "$bc")
contents=$(gen_file_lists "$contents" "$scriptdir/src/dc" "DC" "$dc")
contents=$(gen_file_lists "$contents" "$scriptdir/src/history" "HISTORY" "$hist")
