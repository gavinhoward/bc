...
BcStatus bc_parse_print(BcParse *p, BcVec *code) {
	...
	while (!s && type != BC_LEX_SCOLON && type != BC_LEX_NLINE) {
		...
		if (type == BC_LEX_STRING) {
			...
			if ((s = bc_parse_expr(p, code, 0, bc_parse_next_print)))
				return s;
			s = bc_vec_pushByte(code, BC_INST_PRINT_EXPR);
		}
...
BcStatus bc_parse_return(BcParse *p, BcVec *code) {
	...
	t = p->lex.t.t;
	lprn = t == BC_LEX_LPAREN;

	if (t != BC_LEX_NLINE && t != BC_LEX_SCOLON && !lprn &&
	...
	if (t == BC_LEX_NLINE || t == BC_LEX_SCOLON) {
		...
		if (lprn && (s = bc_lex_next(&p->lex))) return s;

		if (lprn && p->lex.t.t == BC_LEX_RPAREN) {
			s = bc_vec_pushByte(code, BC_INST_RETURN_ZERO);
		}
		else {

			BcParseNext next = lprn ? bc_parse_next_cond : bc_parse_next_expr;

			if ((s = bc_parse_expr(p, code, 0, next))) return s;
			if ((s = bc_vec_pushByte(code, BC_INST_RETURN))) return s;

			if (lprn) {
				if (p->lex.t.t != BC_LEX_RPAREN) return BC_STATUS_PARSE_BAD_EXP;
				s = bc_lex_next(&p->lex);
			}
		}
	}
...
BcStatus bc_parse_init(BcParse *p, BcProgram *program) {
	...
	if ((s = bc_vec_init(&p->ops, sizeof(BcLexToken), NULL))) goto push_err;
...
BcStatus bc_parse_expr(BcParse *p, BcVec *code, uint8_t flags, BcParseNext next)
{
	...
	while (!bcg.signe && !s && !done && bc_parse_token_exprs[t]) {
		...
		switch (t) {
			...
			case BC_LEX_RPAREN:
			{
				...
				else if (!paren_expr) return BC_STATUS_PARSE_BAD_EXP;
	...
	if (flags & BC_PARSE_PRINT) {
		...
		else s = bc_vec_pushByte(code, BC_INST_POP);
	}
